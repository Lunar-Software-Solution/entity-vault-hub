...
const Sidebar = ({
  ...
  const renderMenuItem = (item: MenuItem) => {
    ...
    if (collapsed) {
      return <Tooltip key={item.id}>
          <TooltipTrigger asChild>
            <button onClick={() => onSectionChange(item.id)} className={cn("w-full flex items-center justify-center p-2 rounded-md transition-all duration-200", isActive ? "bg-sidebar-accent text-sidebar-primary" : "text-sidebar-foreground/70 hover:text-sidebar-foreground hover:bg-sidebar-accent/50")}>
    ...
    return <button key={item.id} onClick={() => onSectionChange(item.id)} className={cn("w-full flex items-center gap-2.5 px-3 py-1.5 rounded-md text-sm font-medium transition-all duration-200", isActive ? "bg-sidebar-accent text-sidebar-primary" : "text-sidebar-foreground/70 hover:text-sidebar-foreground hover:bg-sidebar-accent/50")}>
  ...
  const renderGroup = (group: MenuGroup) => {
    ...
    return <Collapsible key={group.id} open={isOpen} onOpenChange={() => toggleGroup(group.id)}>
  ...
  return <TooltipProvider delayDuration={0}>
      <aside className={cn("fixed left-0 top-0 h-screen bg-sidebar border-r border-sidebar-border flex flex-col transition-all duration-300", collapsed ? "w-14" : "w-56")}>
        ...
        <div className={cn("p-2", collapsed && "px-1")}>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="sm" onClick={() => onCollapsedChange?.(!collapsed)} className={cn("w-full justify-center text-muted-foreground hover:text-sidebar-foreground", collapsed ? "px-2" : "px-3")}>
            ...
            {collapsed && <TooltipContent side="right" className="bg-popover text-popover-foreground border border-border">
                Expand sidebar
              </TooltipContent>}
          </Tooltip>
        </div>
        ...
        <div className={cn("p-2 border-t border-sidebar-border", collapsed && "px-1")}>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className={cn("w-full flex items-center gap-2 p-2 rounded-lg hover:bg-sidebar-accent/50 transition-all duration-200", collapsed && "justify-center")}>
                <GravatarAvatar email={userEmail} name={userEmail.split("@")[0]} size="sm" />
                {!collapsed && <>
            ...
            <DropdownMenuContent side={collapsed ? "right" : "top"} align={collapsed ? "end" : "start"} className="w-56 bg-popover border border-border shadow-lg">
              ...
              <DropdownMenuItem onClick={() => setProfileOpen(true)} className="cursor-pointer text-popover-foreground">

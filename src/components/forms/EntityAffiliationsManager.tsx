...
interface EntityAffiliationsManagerProps {
...
const ROLE_TYPES = [
  ...
  { value: "ubo", label: "UBO" },
  { value: "both", label: "Director & UBO" },
];

const EntityAffiliationsManager = ({
  directorId,
  currentEntityId,
  currentEntityName,
}: EntityAffiliationsManagerProps) => {
  const [selectedEntityId, setSelectedEntityId] = useState<string>("");
  const [selectedRoleType, setSelectedRoleType] = useState<string>("director");
  const queryClient = useQueryClient();

  // Fetch all entities for the dropdown
  const { data: entities = [] } = useQuery({
    queryKey: ["entities-for-affiliation"],
    queryFn: async () => {
  ...
  // Fetch existing entity links for this director
  const { data: existingLinks = [], isLoading: isLoadingLinks } = useQuery({
    queryKey: ["director-entity-links", directorId],
    queryFn: async () => {
  ...
  const addLinkMutation = useMutation({
    mutationFn: async ({ entityId, roleType }: { entityId: string; roleType: string }) => {
      if (!directorId) throw new Error("Director ID is required");
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["director-entity-links", directorId] });
      setSelectedEntityId("");
      toast.success("Entity affiliation added");
    },
    onError: (error: any) => {
      if (error.code === "23505") {
        toast.error("This director is already linked to this entity");
      } else {
  ...
  const handleAddLink = () => {
    ...
    addLinkMutation.mutate({ entityId: selectedEntityId, roleType: selectedRoleType });
  };

  // If no director ID yet (new director), show a message
  if (!directorId) {
    return (
      <div className="space-y-3">
        ...
        <div className="p-4 bg-muted/50 rounded-lg border border-dashed text-center">
          <p className="text-sm text-muted-foreground">
            Save this director first to add affiliations with other entities
          </p>
          {currentEntityName && (
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <Badge variant="outline" className="text-xs mt-1">
                    {ROLE_TYPES.find(r => r.value === link.role_type)?.label || link.role_type}
                  </Badge>
                </div>
              </div>
              <Button
...
export default EntityAffiliationsManager;
